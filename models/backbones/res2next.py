...
def bottle2neckx(inputs,
    ...
    if scale != 1 and stype == "normal":
        ...
        sp_ = spx[num_convs]
        if strides != 1:
            sp_ = tf.keras.layers.ZeroPadding2D(((1, 1), (1, 1)), data_format=data_format, name=name + "/pad")(sp_)
            padding = "valid"
...
class Res2NeXt(Model):
    ...
    def build_model(self):
        ...
        self.infilters = 64 
        outputs = [x]
        x = tf.keras.layers.ZeroPadding2D(padding=((1, 1), (1, 1)), self.data_format)(x)
...
@BACKBONES.register("Res2NeXt101")
...
@BACKBONES.register("Res2NeXt152")
