...
class GFLV2Head(GFLHead):
    def __init__(self, cfg, anchor_cfg, num_classes, **kwargs):
        super(GFLV2Head, self).__init__(cfg=cfg, anchor_cfg=anchor_cfg, num_classes=num_classes, **kwargs)
    
        # self._make_shared_convs()
        # self._init_anchor_generators()
        del self.classifier
        self.classifier = tf.keras.layers.Conv2D(
            filters=cfg.num_classes + 1,
            kernel_size=(3, 3),
            strides=(1, 1),
            padding="same",
            use_bias=True,
        ...
        self.quality_predictor = tf.keras.Sequential([
            tf.keras.layers.Conv2D(cfg.quality_filters, 1, 1, name="0"),
            tf.keras.layers.ReLU(name="1"),
            tf.keras.layers.Conv2D(1, 1, 1, name="2"),
            tf.keras.layers.Activation("sigmoid", name="3")
        ], name="reg_conf")

        self._label_dims = self.num_classes if self.use_sigmoid else self.num_classes + 1
    ...
    def call(self, inputs, training=None):
        ...
        for i, level in enumerate(range(self.min_level, self.max_level + 1)):
            ...
            bs = tf.shape(box_feat)[0]
            h, w = tf.shape(box_feat)[1], tf.shape(box_feat)[2]

            pred_boxes = self.regressor(box_feat)
            pred_boxes = self.scales[i](pred_boxes)

            pred_boxes = tf.reshape(pred_boxes, [bs, h, w, 4, self.cfg.reg_max + 1])
            prob = tf.nn.softmax(pred_boxes, -1)

            topk_prob, _ = tf.nn.top_k(prob, self.cfg.reg_topk)
            if self.cfg.add_mean:
                stat = tf.concat([topk_prob, tf.reduce_mean(topk_prob, -1, keepdims=True)], -1)
                last_dim = self.cfg.reg_topk + 1
            else:
                stat = topk_prob
                last_dim = self.cfg.reg_topk
            
            quality_score = self.quality_predictor(tf.reshape(stat, [bs, h, w, 4 * last_dim]))
            
            pred_labels = self.classifier(label_feat)
            pred_labels = tf.nn.sigmoid(pred_labels) * quality_score
    ...
    def compute_losses(self, predictions, image_info):
        with tf.name_scope("compute_losses"):
            ...
            for i, level in enumerate(range(self.min_level, self.max_level+1)):
                ...
                tgt_dist = self.bbox_encoder(tgt_boxes, anchors_xs, anchors_ys)
    ...
    def get_boxes(self, outputs):
        with  tf.name_scope("get_boxes"):
            ...
            for _, level in enumerate(range(self.min_level, self.max_level+1)):
                ...
                pred_labels = tf.reshape(pred_labels, [-1, h * w, self.num_classes + 1])
            ...
            predicted_scores = predicted_labels[:, :, :self.num_classes]  
