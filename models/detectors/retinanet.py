...
class RetinaNet(OneStageDetector):
    def __init__(self, cfg, **kwargs):
        super(RetinaNet, self).__init__(cfg, **kwargs)
...
def _get_weights_from_pretrained(model, pretrained_weights_path):
    ...
    for w in model.weights:
        ...
        if "class" in name:
            name = name.replace("predicted_class.kernel", "retina_cls.weight")
            name = name.replace("predicted_class.bias", "retina_cls.bias")
        if "box" in name and "box_net" not in name:
            name = name.replace("predicted_box.kernel", "retina_reg.weight")
            name = name.replace("predicted_box.bias", "retina_reg.bias")
...
if __name__ == "__main__":
    ...
    # ap_metric = mAP(80)
    # cnt = 0
    # for images, image_info in COCODataset("/home/bail/Data/data1/Dataset/COCO", 
    #                                       augmentations=[dict(Resize=dict(size=(1024, 1024), strides=32, min_scale=1.0, max_scale=1.0))],
    #                                       training=False, 
    #                                       batch_size=4).dataset():
    #     outs = model(images, training=False)
        
    #     num = outs["valid_detections"]
    #     boxes = outs["nmsed_boxes"]
    #     scores = outs["nmsed_scores"]
    #     classes = outs["nmsed_classes"] + 1
    #     gt_boxes = image_info["boxes"]
    #     gt_labels = image_info["labels"]

    #     normalized_factor = tf.convert_to_tensor([tf.shape(images)[1], tf.shape(images)[2]] * 2, gt_boxes.dtype)
    #     boxes *= normalized_factor
    ...
    #         for j in range(n):
    #             img = draw(img, box[j], lbl[j], score[j], coco_id_mapping, random_color(int(lbl[j])))
            
    #         cv2.imwrite("./images/%d.jpg" % cnt, img[:, :, ::-1])
    #         cnt += 1

    #     ap_metric.update_state(gt_boxes, gt_labels, boxes, scores, classes)
