...
@DETECTORS.register
class ATSS(OneStageDetector):
    def __init__(self, cfg, **kwargs):
        super(ATSS, self).__init__(cfg, **kwargs)
...
def _get_weights_from_pretrained(model, pretrained_weights_path):
    ...
    for w in model.weights:
        ...
        if "class" in name:
            name = name.replace("predicted_class.kernel", "atss_cls.weight")
            name = name.replace("predicted_class.bias", "atss_cls.bias")
        if "box" in name and "box_net" not in name:
            name = name.replace("predicted_box.kernel", "atss_reg.weight")
            name = name.replace("predicted_box.bias", "atss_reg.bias")
        if "centerness" in name:
            name = name.replace("predicted_centerness.kernel", "atss_centerness.weight")
            name = name.replace("predicted_centerness.bias", "atss_centerness.bias")
