import time
import tensorflow as tf
from models.builder import DETECTORS
from .one_stage import OneStageDetector


@DETECTORS.register
class FCOS(OneStageDetector):
    def __init__(self, cfg, **kwargs):
        super(FCOS, self).__init__(cfg, **kwargs)
    

_FPN_NAME_MAP = {
    "neck.lateral_convs.5.weight": "module.backbone.fpn.fpn_inner4.weight",
    "neck.lateral_convs.5.bias": "module.backbone.fpn.fpn_inner4.bias",
    "neck.fpn_convs.5.weight": "module.backbone.fpn.fpn_layer4.weight", 
    "neck.fpn_convs.5.bias": "module.backbone.fpn.fpn_layer4.bias",
    "neck.lateral_convs.4.weight": "module.backbone.fpn.fpn_inner3.weight", 
    "neck.lateral_convs.4.bias": "module.backbone.fpn.fpn_inner3.bias",
    "neck.fpn_convs.4.weight": "module.backbone.fpn.fpn_layer3.weight",
    "neck.fpn_convs.4.bias": "module.backbone.fpn.fpn_layer3.bias",
    "neck.lateral_convs.3.weight": "module.backbone.fpn.fpn_inner2.weight", 
    "neck.lateral_convs.3.bias": "module.backbone.fpn.fpn_inner2.bias",  
    "neck.fpn_convs.3.weight": "module.backbone.fpn.fpn_layer2.weight",
    "neck.fpn_convs.3.bias": "module.backbone.fpn.fpn_layer2.bias",
    "neck.fpn_convs.6.weight": "module.backbone.fpn.top_blocks.p6.weight",
    "neck.fpn_convs.6.bias": "module.backbone.fpn.top_blocks.p6.bias",
    "neck.fpn_convs.7.weight": "module.backbone.fpn.top_blocks.p7.weight",
    "neck.fpn_convs.7.bias": "module.backbone.fpn.top_blocks.p7.bias",
}



def _get_weights_from_pretrained(model, pretrained_weights_path):
    import torch
    import numpy as np

    pretrained = torch.load(pretrained_weights_path, map_location="cpu")["model"]
    # for k, v in pretrained.items():
    #     print(k, v.numpy().shape)
    
    for w in model.weights:
        name = w.name.split(":")[0]
        name = name.replace("/", ".")
        
        if "shortcut.conv2d.kernel" in name:
            name = name.replace("shortcut.conv2d.kernel", "downsample.0.weight")
        if "shortcut.batch_norm" in name:
            name = name.replace("shortcut.batch_norm", "downsample.1")
        if "conv2d.kernel" in name:
            name = name.replace("conv2d.kernel", "weight")
        if "conv2d.bias" in name:
            name = name.replace("conv2d.bias", "bias")
        if "kernel" in name:
            name = name.replace("kernel", "weight")
        if "gamma" in name:
            name = name.replace("gamma", "weight")
        if "beta" in name:
            name = name.replace("beta", "bias")
        if "moving_mean" in name:
            name = name.replace("moving_mean", "running_mean")
        if "moving_variance" in name:
            name = name.replace("moving_variance", "running_var")
        if "stem" in name or "layer" in name:
            name = "module.backbone.body." + name
        if "conv1.batch_norm" in name:
            name = name.replace("conv1.batch_norm", "bn1")
        if "conv2.batch_norm" in name:
            name = name.replace("conv2.batch_norm", "bn2")
        if "conv3.batch_norm" in name:
            name = name.replace("conv3.batch_norm", "bn3")
        if name in _FPN_NAME_MAP:
            name = _FPN_NAME_MAP[name]
        if "head" in name:
            name = "module.rpn." + name
        if "box_net" in name:
            name = name.replace("box_net", "bbox_tower")
        if "cls_net" in name:
            name = name.replace("cls_net", "cls_tower")
        if "class" in name:
            name = name.replace("predicted_class", "cls_logits")
        if "box" in name and "bbox_tower" not in name:
            name = name.replace("predicted_box", "bbox_pred")
        if "predicted_centerness":
            name = name.replace("predicted_centerness", "centerness")
        if "scale" in name:
            name = name.replace("head", "head.scales")
           
        # print(name, w.shape.as_list())
        pw = pretrained[name]
        if len(pw.shape) == 4:
            pw = np.transpose(pw, [2, 3, 1, 0])
        if len(pw.shape) == 2:
            pw = np.transpose(pw, [1, 0])
        if "scale" in name:
            pw = pw[0]
            
        w.assign(pw)


if __name__ == "__main__":
    import cv2
    import numpy as np
    from demo import coco_id_mapping, draw, random_color, preprocess
    from configs.fcos_config import get_fcos_config
    from data.datasets.coco_dataset import COCODataset
    from core.metrics.mean_average_precision import mAP

    cfg = get_fcos_config()
    model = FCOS(cfg, training=False)
    # model.neck.summary()
    # model.detector.summary()
    # model.detector(tf.random.uniform((1, 512, 512, 3)), training=False)
    torch_weight_name = "FCOS_imprv_R_50_FPN_1x"
    _get_weights_from_pretrained(model.detector, "/home/bail/Data/data2/pretrained_weights/%s.pth" % torch_weight_name)

    img = cv2.imread("/home/bail/Workspace/TRTNets/images/bus.jpg")
    img, _, _, _ = preprocess(img, 1024)
    img = img[:, :, ::-1]
    inp = tf.convert_to_tensor(img[None], tf.float32)
    inp = tf.concat([inp, inp], 0)
    
    outs = model(inp, training=False)
    
    num = outs["valid_detections"].numpy()[1]
    boxes = outs["nmsed_boxes"].numpy()[1]
    scores = outs["nmsed_scores"].numpy()[1]
    classes = outs["nmsed_classes"].numpy()[1]
    
    for i in range(num):
        box = boxes[i]
        # if scores[i] < 0.5:
        #     continue
        # box = boxes[i] * np.array([height, width, height, width])
        c = classes[i] + 1
        print(box, c)
        img = draw(img, box, c, scores[i], coco_id_mapping, random_color(int(c)))
    
    cv2.imshow("img", img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    tf.saved_model.save(model.detector, "/home/bail/Data/data2/pretrained_weights/%s" % torch_weight_name)
    model.save_weights("/home/bail/Data/data2/pretrained_weights/%s.h5" % torch_weight_name)

