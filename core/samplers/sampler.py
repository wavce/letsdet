...
class Sampler(metaclass=ABCMeta):
    def __init__(self, num_proposals, pos_fraction, neg_pos_ub=-1, add_gt_as_proposals=False, **kwargs):
    ...
    def sample(self, assigned_boxes, assigned_labels, gt_boxes=None, gt_labels=None, **kwargs):
        ...
        """
        if self.add_gt_as_proposals and gt_boxes is not None:
        ...
        box_weights = tf.tensor_scatter_nd_update(
            box_weights, pos_inds[:, None], tf.ones_like(pos_inds, box_weights.dtype))
        label_weights = tf.tensor_scatter_nd_update(
            box_weights, neg_inds[:, None], tf.ones_like(neg_inds, box_weights.dtype))

        # target_labels = tf.where(label_weights >= 1, assigned_labels, tf.zeros_like(assigned_labels))
        # box_weights = tf.expand_dims(box_weights, -1)  

        return assigned_boxes, assigned_labels, box_weights, label_weights
    
    def __call__(self, assigned_boxes, assigned_labels, gt_boxes=None, gt_labels=None, **kwargs):
        with tf.name_scope("sample"):
            return self.sample(assigned_boxes, assigned_labels, gt_boxes=gt_boxes, gt_labels=gt_labels, **kwargs)
